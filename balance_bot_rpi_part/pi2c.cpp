...
int16_t* Pi2c::i2cReadArduinoInt(){
	const int arr_size = 2;
	char tmp[arr_size];
	static int16_t retval[2]; // static int retval[arr_size];
	
	if (i2cRead(tmp,arr_size) > 0){
		
		retval[0] = (tmp[0] | tmp[1] << 8 ); //Using bit shifting, turn the 2 byte array into an Int.
		//printf("rp1: %d rp2: %d \n", retval[0], retval[2]);
...
int Pi2c::i2cWriteArduino2Int(int input1, int input2){
	...
	tmp[0] = input1; //get lowest 8 bits into the first part of the array;
	tmp[1] = input1 >> 8; //get the highest 8 bits into the second part of the array;
	
	tmp[2] = input2; //get lowest 8 bits into the first part of the array;
	tmp[3] = input2 >> 8; //get the highest 8 bits into the second part of the array;
	retval = (i2cWrite(tmp,arr_size) > 0);
	/*
	tmp[0] = input2; //get lowest 8 bits into the first part of the array;
	tmp[1] = input2 >> 8; //get the highest 8 bits into the second part of the array;
	retval2 = (i2cWrite(tmp,arr_size) > 0);
	
	if(retval1 >0 && retval2 >0) retval = 1;
	*/
	return retval;
}
